...
export default function AboutPage() {
  ...
  return (
    <main className="min-h-screen">
      ...
      {/* Differentiators */}
      <section className="py-40 bg-slate-950/50 border-y border-white/5 relative">
        <div className="max-w-7xl mx-auto px-6 relative z-10">
          <div className="grid lg:grid-cols-2 gap-32 items-center">
            <div className="space-y-12">
              <h2 className="text-5xl font-black tracking-tighter text-white uppercase leading-[0.9] mb-10">
                Engineering <br /><span className="text-blue-600">Over Generation.</span>
              </h2>
              <p className="text-xl text-slate-500 font-medium leading-relaxed">
                PromptFlow is not just a collection of text. It is a production system for high-end visual assets. 
                We provide the logic behind every prompt, ensuring scalability for pro creators.
              </p>
              
              <div className="grid gap-10">
                {[
                  { icon: Target, title: 'Variable Logic', desc: 'Precise control over subject, lighting, and composition.' },
                  { icon: Layers, title: 'Cross-Model Adaptability', desc: 'Optimized tips for Midjourney, DALL-E, and SDXL.' },
                  { icon: Lightbulb, title: 'Strategic Analysis', desc: 'Understanding why a prompt works is better than blind copying.' },
                  { icon: Lock, title: 'No-API Commitment', desc: 'We never ask for keys. You control the generation on your tools.' }
                ].map((feature) => (
                  <div key={feature.title} className="flex gap-8 items-start group">
                    <div className="w-14 h-14 rounded-2xl bg-blue-600/5 border border-blue-600/10 flex items-center justify-center text-blue-500 group-hover:bg-blue-600/10 group-hover:border-blue-600/20 transition-all shrink-0">
                      <feature.icon size={24} />
                    </div>
                    <div>
                      <h3 className="text-base font-black text-white uppercase tracking-[0.2em] mb-2">{feature.title}</h3>
            ...
            <div className="relative aspect-square glass rounded-[4rem] p-16 border-white/10 flex items-center justify-center glow-blue">
               <div className="absolute inset-0 bg-blue-600/5 rounded-full blur-[120px] -translate-x-1/4 -translate-y-1/4" />
               <div className="relative z-10 w-full space-y-10">
                  <div className="h-3 w-32 bg-blue-600 rounded-full glow-primary" />
                  <div className="space-y-6">
                    <div className="h-5 w-full bg-white/5 rounded-full" />
                    <div className="h-5 w-4/5 bg-white/5 rounded-full" />
                    <div className="h-5 w-3/5 bg-white/5 rounded-full" />
                  </div>
                  <div className="pt-10">
                    <div className="aspect-[16/10] glass rounded-3xl border-white/10 flex items-center justify-center">
                      <Zap className="text-blue-500 opacity-20" size={64} strokeWidth={1} />
                    </div>
                  </div>
               </div>
            </div>
        ...
        <div className="max-w-7xl mx-auto px-6 text-center">
          ...
          <div className="flex flex-col md:flex-row items-center justify-between gap-12 pt-20 border-t border-white/5">
            <div className="flex items-center gap-3">
              ...
              <span className="text-lg font-black tracking-tighter text-white uppercase">PROMPTFLOW</span>
            </div>
            
            <p className="text-[10px] font-black text-slate-700 uppercase tracking-[0.5em]">
              © 2026 PromptFlow Protocol. All systems optimal.
            </p>
          </div>
        </div>
      </footer>
