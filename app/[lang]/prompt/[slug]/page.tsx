...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  return {
    ...
    alternates: {
      canonical: `/${params.lang}/prompt/${prompt.slug}`,
      languages: {
        'en': `/en/prompt/${prompt.slug}`,
        'ko': `/ko/prompt/${prompt.slug}`,
      },
    },
    openGraph: {
      title: prompt.title,
...
export function generateStaticParams() {
  ...
  locales.forEach(lang => {
    prompts.forEach(prompt => {
      params.push({ lang, slug: prompt.slug });
    });
  });
...
export default function PromptDetailPage({ params: { lang, slug } }: Props) {
        ...
        <div className="max-w-7xl mx-auto px-6 h-20 flex items-center justify-between">
          ...
          <div className="flex items-center gap-6">
            <div className="flex items-center gap-2 border-r border-white/10 pr-6 mr-2">
              <Link href={`/en/prompt/${slug}/`} className={`text-[10px] font-black uppercase tracking-widest ${lang === 'en' ? 'text-blue-500' : 'text-slate-500'}`}>EN</Link>
              <span className="text-slate-800 text-[10px]">/</span>
              <Link href={`/ko/prompt/${slug}/`} className={`text-[10px] font-black uppercase tracking-widest ${lang === 'ko' ? 'text-blue-500' : 'text-slate-500'}`}>KO</Link>
            </div>
        ...
        <div className="grid lg:grid-cols-2 gap-24 items-start mb-32">
          <div className="space-y-10">
            <div className="flex flex-wrap gap-3">
              {prompt.verified_badge && (
                <div className="flex items-center gap-1.5 px-4 py-1.5 rounded-full bg-emerald-500/10 border border-emerald-500/20 text-emerald-400 text-[9px] font-black uppercase tracking-[0.2em]">
                  <ShieldCheck size={14} /> {dict.prompt.verified}
                </div>
              )}
            ...
            <p className="text-2xl text-slate-400 leading-relaxed font-medium">
              {lang === 'ko' && prompt.result_first_title_ko ? prompt.result_first_title_ko : prompt.result_first_title}
            </p>
            ...
            <div className="p-8 glass rounded-[2.5rem] border-l-4 border-l-blue-600 relative overflow-hidden">
              <div className="relative z-10">
                <p className="text-xl text-white font-bold italic leading-relaxed">
                  "{lang === 'ko' && prompt.emotional_hook_ko ? prompt.emotional_hook_ko : prompt.emotional_hook}"
                </p>
              </div>
            ...
            <div className="space-y-6">
              <h3 className="text-[10px] font-black text-slate-500 uppercase tracking-[0.4em] flex items-center gap-3">
                <Target size={16} /> {dict.prompt.scenarios}
              </h3>
              <div className="flex flex-wrap gap-3">
                {(lang === 'ko' && prompt.use_cases_ko ? prompt.use_cases_ko : prompt.use_cases).map((useCase, idx) => (
                  <span
                    key={idx}
          ...
          <div className="lg:col-span-7 space-y-24">
            ...
            <section className="space-y-12">
              <div className="flex items-center gap-4">
                ...
                <h3 className="text-2xl font-black text-white tracking-tighter uppercase">{dict.prompt.why}</h3>
              </div>
              <div className="grid gap-6">
                {(lang === 'ko' && prompt.why_this_works_ko ? prompt.why_this_works_ko : prompt.why_this_works).map((reason, idx) => (
            ...
            {prompt.variations.length > 0 && (
              <section className="space-y-12">
                <div className="flex items-center gap-4">
                  ...
                  <h3 className="text-2xl font-black text-white tracking-tighter uppercase">{dict.prompt.variations}</h3>
                </div>
                <div className="grid gap-6">
                  {prompt.variations.map((variation) => (
                    ...
                    >
                      <div className="flex items-center justify-between mb-6">
                        <h4 className="text-xl font-black text-white uppercase tracking-tight group-hover:text-blue-400 transition-colors">
                          {lang === 'ko' && variation.label_ko ? variation.label_ko : variation.label}
                        </h4>
                        <span className="text-[10px] font-black text-slate-500 uppercase tracking-widest">
                          {lang === 'ko' && variation.use_case_ko ? variation.use_case_ko : variation.use_case}
                        </span>
                      </div>
                      <div className="bg-slate-950 p-6 rounded-2xl border border-white/10 font-mono text-sm text-blue-400/80 leading-relaxed">
                        <span className="text-slate-700 block mb-2 uppercase text-[9px] font-black tracking-widest">{dict.prompt.modification}</span>
          ...
          <div className="lg:col-span-5 space-y-16">
            <section className="glass rounded-[2.5rem] p-10 space-y-12 sticky top-32">
              <div className="flex items-center gap-4">
                ...
                <h3 className="text-sm font-black text-white uppercase tracking-[0.4em]">{dict.prompt.optimization}</h3>
              </div>
              ...
              <div className="space-y-10">
                ...
                <p className="text-sm text-slate-500 leading-relaxed font-bold">
                  {lang === 'ko' && note.note_ko ? note.note_ko : note.note}
                </p>
                  </div>
                ))}
              </div>
              ...
              {prompt.comparison_notes && (
                <div className="pt-10 border-t border-white/5">
                  <h4 className="text-[10px] font-black text-slate-600 uppercase tracking-[0.3em] mb-6 flex items-center gap-2">
                    <Star size={14} className="text-yellow-500/50" /> {dict.prompt.insights}
                  </h4>
                  <p className="text-sm text-slate-400 leading-relaxed font-bold italic">
                    "{lang === 'ko' && prompt.comparison_notes_ko ? prompt.comparison_notes_ko : prompt.comparison_notes}"
                  </p>
                </div>
              )}
        ...
        <section className="mt-40 pt-24 border-t border-white/5">
          <div className="flex items-center justify-between mb-16">
            <div className="flex items-center gap-4">
              ...
              <h3 className="text-2xl font-black text-white uppercase tracking-tighter">{dict.prompt.related}</h3>
            </div>
            <Link 
              href={`/${lang}/category/${prompt.category}/`}
            ...
            >
              {dict.nav.view_all} <ChevronRight size={14} />
            </Link>
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-12">
            {prompts
              ...
              .slice(0, 3)
              .map(p => (
                <PromptCard key={p.id} prompt={p} lang={lang} />
              ))
            }
